...
function App() {
  ...
  const totals = useMemo(() => {
    ...
    return { totalCost, selling, margin };
  }, [rfq, app, pump, crane]);
      ...
      <div style={S.wrap}>
        <div style={S.header}>
          <div>
            ...
            <div style={S.small}>RFQ, costing, technical spec library, drawing library and deal tracker.</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button style={S.button} onClick={saveDeal}>Save Deal</button>
        ...
        <div style={S.tabs}>
          {["rfq", "quote", "technical", "drawings", "deals", "library"].map((x) => (
            <button key={x} onClick={() => setTab(x)} style={{ ...S.tab, ...(tab === x ? S.active : {}) }}>
              {x === "rfq" ? "RFQ Configurator" : x === "quote" ? "Quote Preview" : x === "technical" ? "Technical Spec" : x === "drawings" ? "Drawings" : x === "deals" ? "Deal Tracker" : "Libraries"}
            </button>
          ))}
        </div>
    ...
    <div style={S.grid}>
      <div style={S.card}>
        ...
        <div style={S.formGrid}>
          ...
          <Field label="Margin %" type="number" value={rfq.margin} onChange={(v) => update("margin", v)} />
      ...
      <div style={S.card}>
        ...
        <Cost label="Total Cost" value={totals.totalCost} bold />
        <div style={{ marginTop: 18, padding: 18, borderRadius: 14, background: "#eef2ff" }}>
          <div style={S.small}>Selling Price</div>
          <div style={{ fontSize: 28, fontWeight: 900 }}>KWD {totals.selling.toFixed(3)}</div>
          <span style={S.badge}>Margin {totals.margin}%</span>
        </div>
...
function Quote({ rfq, app, pump, crane, tech, totals }) {
  return (
    <div style={S.card}>
      ...
      <table style={S.table}><tbody>
        <Row label="Base Fabrication" value={`KWD ${app.baseCost.toFixed(3)}`} />
        <Row label="Pump / Hydraulic System" value={`KWD ${pump.cost.toFixed(3)}`} />
        <Row label="Crane Installation" value={`KWD ${crane.cost.toFixed(3)}`} />
        <Row label="Accessories & Labor" value={`KWD ${(Number(rfq.accessories || 0) + Number(rfq.labor || 0)).toFixed(3)}`} />
        <Row label="Total Offer Price" value={`KWD ${totals.selling.toFixed(3)}`} />
      </tbody></table>
    </div>
  );
}
...
function Drawings({ rfq, tech }) {
  return (
    <div style={S.card}>
      ...
      <p style={S.small}>Replace these placeholders later with actual PDF/JPG/PNG engineering drawings.</p>
...
function Deals({ deals }) {
  return (
    <div style={S.card}>
      <h2>Deal Tracker</h2>
      <table style={S.table}>
        <thead><tr><th style={S.th}>Quote No</th><th style={S.th}>Date</th><th style={S.th}>Customer</th><th style={S.th}>Application</th><th style={S.th}>Value</th><th style={S.th}>Status</th></tr></thead>
        <tbody>{deals.map((d) => <tr key={d.id}><td style={S.td}>{d.quoteNo}</td><td style={S.td}>{d.date}</td><td style={S.td}>{d.customer}</td><td style={S.td}>{d.application}</td><td style={S.td}>KWD {d.value.toFixed(3)}</td><td style={S.td}><span style={S.badge}>{d.status}</span></td></tr>)}</tbody>
      </table>
      {deals.length === 0 && <p style={{ textAlign: "center", color: "#64748b" }}>No deals saved yet.</p>}
    </div>
  );
}
